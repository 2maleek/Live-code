...
function checkAdmission (student) {
  ...
  var studentId = '';
  studentId +=[student.date[8] + student.date[9] + student.date[3] + student.date[4] + student.dob[8] + student.dob[9] + student.dob[3] + student.dob[4]];
  console.log(studentId);

  //cek status seleksi
  var status = 0;
  for(var i=0; i<student.scores.length; i++) {
    status += student.scores[i];
  }
  //dapatkan nilai rata-rata
  status = status / student.scores.length;
  //seleksi status reject or accept
  if(status < 70) {
    status = 'rejected'
  }else {
    status = 'accepted'
